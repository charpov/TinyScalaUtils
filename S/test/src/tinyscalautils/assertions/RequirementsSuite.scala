...
class RequirementsSuite extends AnyFunSuite:
   ...
   test("requireElement basic"):
      requireElement(true)
      val ex = intercept[NoSuchElementException](requireElement(false))
      assert(ex.getMessage eq null)

   test("requireElement lazy message 1"):
      var count  = 0
      def incr() =
         count += 1
         count
      ...
      val ex = intercept[NoSuchElementException](requireElement(count == 1, s"${incr()}"))
      assert(count == 1)
      assert(ex.getMessage == "1")

   test("requireElement lazy message 2"):
      var count  = 0
      def incr() =
         count += 1
         count

      requireElement(count == 0, "%d%d", incr(), incr())
      assert(count == 0)

      val ex = intercept[NoSuchElementException](requireElement(count == 1, "%d%d", incr(), incr()))
      assert(count == 2)
      assert(ex.getMessage == "12")
