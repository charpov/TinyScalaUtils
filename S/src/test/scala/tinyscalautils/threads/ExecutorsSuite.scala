...
import java.util.concurrent.{ ConcurrentLinkedQueue, RejectedExecutionException, ThreadFactory }
...
class ExecutorsSuite extends AnyFunSuite with Tolerance:
   ...
   test("keepalive") {
      ...
      10 times exec.execute((() => threads.add(Thread.currentThread)): Runnable)
