...
import tinyscalautils.test.tagobjects.Points.{ points, pt }
import tinyscalautils.test.tagobjects.{ Async, Fail, Points }
...
class GradingTests extends AnyFunSuite with Tolerance:
   ...
   test("Point tag"):
      class Tests extends AnyFunSuite with Grading:
         test("failed [3 points]")(fail())
         test("test 1", Points(4))(unit)
         test("test 2", (1 + 1).points)(unit)
         test("test 3", 1.pt)(unit)
      end Tests

      val suite = Tests()
      suite.run(None, silent)
      assert(suite.grader.grade == 0.7)
